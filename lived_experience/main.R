...
library(here)
source("lived_experience/scrape_scholar_function2.R")
source("helper_functions.R")

###--- Files & folders
output_folder <- "lived_experience/output"

###--- Data
jou_auth <- readRDS("lived_experience/journal_tbl.RDS")
...
###--- Done
  ...
  keyword <- x[2]
  journal_site <- x[3]
  tibble(journal_abb,keyword,journal_site)
}) |> 
  bind_rows()
...
for(j in 1:nrow(jou_auth)){
  ...
  temp_output <- paste0("lived_experience/temp/",output)
